...
const displayCategoryBtn = (data)=>{
 ...
 data.map((item)=>{
    const div = createDiv();
    div.innerHTML = `<div class="flex justify-between gap-2 items-center border-2 rounded-lg w-fit px-4 py-2 md:px-12 md:py-4 cursor-pointer hover:rounded-full hover:bg-[#0E7A811A] transition-all ease-linear duration-300" id='${item.category}_btn'>
                <div>
...
const spinnerHandler = async ()=>{
     ...
     spinner.classList = "justify-center items-center w-full";

     setTimeout(()=>{
        spinner.classList = "justify-center items-center w-full hidden";
     }, 2000);
